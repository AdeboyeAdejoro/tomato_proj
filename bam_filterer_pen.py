...
bamfile_dir_path = "/nam-99/ablage/nam/adejoro_folder/bamfiles_folder_pen"
corresponding_table_dir_path = "/nam-99/ablage/nam/adejoro_folder/ILSolanum/written_files_restructured"
output_dir_path = "/nam-99/ablage/nam/adejoro_folder/filtered_bam_folder_for_pen"
...
for bam_file in tqdm(bam_files):
    ...
    print(f"current csv_file is {csv_file}")
    ...
    second_level_interpolated_name = interpolated_name.replace(".bam", "_filtered_pen.bam")
    ...
    if not os.path.exists(output_bamfile_path):
        ...
        for read in bamfile.fetch():
            ...
            if read_id in mapping_table and mapping_table[read_id] == (0, 1):
                print(f"Adding read: {read_id} to {output_bamfile}")
                print(f"The tuple is: {mapping_table[read_id]}")
